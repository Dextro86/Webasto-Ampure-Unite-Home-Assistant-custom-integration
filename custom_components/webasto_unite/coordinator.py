...
class WebastoUniteCoordinator(DataUpdateCoordinator[RuntimeSnapshot]):
    ...
    async def _async_update_data(self) -> RuntimeSnapshot:
        try:
            wallbox = await self._read_wallbox_state()
            if (
    ...
    async def _read_wallbox_state(self) -> WallboxState:
        ...
        try:
            ...
            wallbox.session_start_time = self._format_clock_hhmmss(await self.client.read(SESSION_START_TIME))
            wallbox.session_duration_s = int(await self.client.read(SESSION_DURATION_S))
            wallbox.session_end_time = self._format_clock_hhmmss(await self.client.read(SESSION_END_TIME))
            ...
            wallbox.charging_state = self._map_charging_state(charge_point_state)
            ...
            configured_phases = self.entry.data.get(CONF_INSTALLED_PHASES, "3p")
            if configured_phases == "1p":
                wallbox.installed_phases = 1
            else:
                wallbox.installed_phases = 3
    ...
    @staticmethod
    def _format_clock_hhmmss(raw_value: float | int | None) -> str | None:
    ...
    def _map_charging_state(self, raw_state: int) -> ChargingState:
        mapping = {
    ...
    def _read_sensor_snapshot(self) -> HaSensorSnapshot:
        ...
        if self.control_config.dlb_input_model == DlbInputModel.PHASE_CURRENTS:
            snapshot.phase_currents = PhaseCurrents(
                l1=self._state_as_current_a(options.get(CONF_DLB_L1_SENSOR)),
                l2=self._state_as_current_a(options.get(CONF_DLB_L2_SENSOR)),
                l3=self._state_as_current_a(options.get(CONF_DLB_L3_SENSOR)),
            )
        else:
            snapshot.grid_power_w = self._state_as_power_w(options.get(CONF_DLB_GRID_POWER_SENSOR))

        if self.control_config.pv_input_model == PvInputModel.SURPLUS_SENSOR:
            snapshot.surplus_power_w = self._state_as_power_w(options.get(CONF_PV_SURPLUS_SENSOR))
        else:
    ...
    def _state_as_current_a(self, entity_id: str | None) -> float | None:
        return self._state_as_float(entity_id, expected_kind="current")

    def _state_as_power_w(self, entity_id: str | None) -> float | None:
        return self._state_as_float(entity_id, expected_kind="power")

    def _state_as_float(self, entity_id: str | None, expected_kind: str) -> float | None:
        if not entity_id:
            return None
        ...
        return self._normalize_sensor_value(entity_id, expected_kind, value, unit)
