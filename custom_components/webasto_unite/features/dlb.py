...
class DlbEngine:
    ...
    def _from_phase_currents(
        ...
        if any(grid_currents[idx] is None for idx in active_indices):
            return DlbResult(self.config.safe_current_a, False, ControlReason.SENSOR_UNAVAILABLE)
        available = min(fuse - float(grid_currents[idx]) + charger_currents[idx] - margin for idx in active_indices)
    ...
    def _active_phase_indices(self, charger_phase_currents: PhaseCurrents | None) -> list[int]:
        if charger_phase_currents is None:
            return []
        indices: list[int] = []
        for idx, value in enumerate((charger_phase_currents.l1, charger_phase_currents.l2, charger_phase_currents.l3)):
