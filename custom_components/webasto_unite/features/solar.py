...
def resolve_installed_phase_count(wallbox: WallboxState) -> int:
    """Return the configured installation phase count used for calculations."""
    if wallbox.installed_phases in (1, 3):
        return wallbox.installed_phases
    return 3


def resolve_solar_phase_context(
    *,
    mode: ChargeMode,
    wallbox: WallboxState,
    strategy: SolarControlStrategy,
    installed_phases: int,
    observed_session_phase_count: int | None,
) -> tuple[int, str]:
    """Resolve the phase count Solar should use for current calculations.

    This is calculation context only. It does not decide or execute phase
    switching.
    """
    ...
    if (
        ...
        and observed_session_phase_count in (1, 3)
    ):
        return observed_session_phase_count, "observed_session_phases"
    ...
    if (
        ...
        if normalize_solar_control_strategy(strategy) == SolarControlStrategy.ECO_SOLAR:
