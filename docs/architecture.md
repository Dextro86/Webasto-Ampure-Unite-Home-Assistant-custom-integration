...
Phase restore is conservative: live unplug only clears integration runtime/session state and does not write register `405`, because the charger may still be closing the session internally. After unplug, the integration schedules a short delayed Modbus reconnect so the next plug-in session starts from a fresh TCP socket. A new settled non-Solar managed session may restore configured 3P if register `405` is still 1P. Restart paths still do not treat the first read as a fresh plug-in event. Use the manual `Restore Configured Phase` control if you want to write the configured phase mode explicitly.
